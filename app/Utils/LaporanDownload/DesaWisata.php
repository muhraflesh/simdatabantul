...
class DesaWisata {
    public function downloadOperatorDesa($req, $data){
        ...
        Excel::create("Laporan_Wisatawan_".ucwords(\Auth::user()->pengelola[0]->wisata->nama)."_".$waktu, function($excel) use ($waktu, $date, $data, $req){
            ...
            $excel->sheet(ucwords($req->waktuTipe)." - ".str_replace('/', '', $date), function($sheet) use ($date, $data, $req){
                $head1 = "Data Wisatawan Desa Wisata ".ucwords(\Auth::user()->pengelola[0]->wisata->nama);
                ...
                if($req->waktuTipe=='tahunan'){
                    $jmlColWaktu = 11;
                    $head3 = ['No','Desa Wisata','Kecamatan','Wisatawan','Bulan','','','','','','','','','','','','Total'];
                }elseif($req->waktuTipe=='bulanan'){
                ...
                foreach($data as $data){
                    $b1 = [$no, $data->wisata->nama, $data->wisata->kelurahan->kecamatan->nama, 'Wisman'];
                    $b2 = ['', '', '', 'Wisnus'];
                    $b3 = ['', '', '', 'Jumlah'];
                    ...
                    $dataStatistiks = $this->getStatistikDesaWisata($data->wisata_id, $req->waktuTipe, $waktunaa);
                    // inisiasi
    ...
    public function downloadDesa($req, $data){

        $waktu = ($req->waktuTipe=='bulanan') ? $req->waktuBulan."-".$req->waktuTahun : $req->waktu;        
        $date = $waktu;
       
        Excel::create("Laporan_DesaWisata_".$waktu, function($excel) use ($waktu, $date, $data, $req){
            // Set the title
            $title = "Laporan_DesaWisata_".$waktu;
            ...
            $excel->sheet(ucwords($req->waktuTipe)." - ".str_replace('/', '', $date), function($sheet) use ($date, $data, $req){
                ...
                $head1 = "Data Wisatawan Desa Wisata";
    ...
    static function getStatistikDesaWisata($wisataId, $jenisLaporan="tahunan", $waktu="2019-08-31"){
