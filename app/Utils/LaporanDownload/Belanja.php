...
class Belanja {
    public function downloadOperatorBelanja($req, $data){
        ...
        Excel::create("Laporan_Belanja_".ucwords(\Auth::user()->pengelola[0]->wisata->nama)."_".$waktu, function($excel) use ($waktu, $date, $data, $req){
            // Set the title
            $title = "Laporan_Belanja_".ucwords(\Auth::user()->pengelola[0]->wisata->nama)."_".$waktu;
            ...
            $excel->sheet(ucwords($req->waktuTipe)." - ".str_replace('/', '', $date), function($sheet) use ($date, $data, $req){
                $head1 = "Data Wisatawan Belanja di ".ucwords(\Auth::user()->pengelola[0]->wisata->nama);
                ...
                if($req->waktuTipe=='tahunan'){
                    $jmlColWaktu = 11;
                    $head3 = ['No','Nama Desa Wisata','Jenis Belanja','Wisatawan','Bulan','','','','','','','','','','','','Total'];
                }elseif($req->waktuTipe=='bulanan'){
                    $a_date = $req->waktuTahun."-".$req->waktuBulan."-01";                    
                    $jmlColWaktu = date('t', strtotime($a_date))-1;
                    $head3 = ['No','Nama Desa Wisata','Jenis Belanja','Wisatawan','Tanggal'];
                    ...
                    $jmlColWaktu = 4;
                    $head3 = ['No','Nama Desa Wisata','Jenis Belanja','Wisatawan','Minggu Ke','','','','','Total'];
                }elseif($req->waktuTipe=='harian'){
                    $jmlColWaktu = 0;
                    $head3 = ['No','Nama Desa Wisata','Jenis Belanja','Wisatawan','Tanggal','Total'];
                }
                ...
                foreach($data as $data){
                    ...
                    $dataStatistiks = $this->getStatistikBelanja($data->wisata_id, $req->waktuTipe, $waktunaa);
    ...
    public function downloadBelanja($req, $data){
        ...
        Excel::create("Laporan_Belanja_".$waktu, function($excel) use ($waktu, $date, $data, $req){
            // Set the title
            $title = "Laporan_Belanja_".$waktu;
    ...
    static function getStatistikBelanja($wisataId, $jenisLaporan="tahunan", $waktu="2019-08-31"){

        $model = \App\Belanja::where('wisata_id', $wisataId);
        ...
        if($jenisLaporan=='tahunan'){
            ...
            $weekNumber = Belanja::weekOfMonth($lastDate);
