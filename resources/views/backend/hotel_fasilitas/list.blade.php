...
@section('breadcrumbs')
    {!! Breadcrumbs::render('list_fasilitas_umum') !!}
@endsection

@section('action')
<div class="col s2 m6 l6">
    <a class="btn waves-effect waves-light breadcrumbs-btn right" href="{{ route('backend::hotel_fasilitas.create') }}">
...
<!-- <div class="col s2 m6 l6">
    ...
    <ul class="dropdown-content" id="dropdown1" tabindex="0">
        <li tabindex="0">
            <a class="grey-text text-darken-2" href="{{ route('backend::hotel_fasilitas.create') }}">
                Tambah Data
            </a>
        </li>
    ...
    <div class="col s12 m12 l12">
        <div id="responsive-table" class="card card card-default scrollspy">
            <div class="card-content">
                ...
                @endif
                <form class="col s12" action="{{ route('backend::hotel_fasilitas.index') }}" method="GET">
                    <div class="row">
                        <div class="input-field col m4 s6">
                            <input type="text" name="nama" value="{{ \Request::get('nama') }}" class="form-control form-control-sm" placeholder="Nama Fasilitas" />
                        </div>
            ...
            <div class="row">
                ...
                <table class="responsive-table">
                    ...
                    <tr>
                        ...
                        <th data-field="nama">Nama Fasilitas</th>
                        <th data-field="jumlah">Jumlah Fasilitas</th>
                    ...
                    <?php $no=1;?>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                $potonas = (!empty($res->fotos->first())) ? json_encode($res->fotos) : "[]";
                                @endphp
                                <img src="{{ asset($potona) }}" class="foto" width="100px" onclick="zoomFoto('{{ $potonas }}')" />
                            </td>
                            <td>
                                {{ $res->nama }}
                            </td>
                            ...
                            <td>
                                ...
                                <ul id='dropdown{{ $no }}' class='dropdown-content'>
                                    ...
                                    <li>
                                        <a href="{{ route('backend::hotel_fasilitas.edit', ['id' => $res->id]) }}">
...
function zoomFoto(datas){
    console.log(datas);
    fotos = JSON.parse(datas)
    $('#zoomFoto').modal('open');
    // $('#srcZoomFoto').attr('src', url);
    htmlData = "";
    fotos.forEach(function(item, index){
        htmlData += "<a class=\"carousel-item\" target=\"_blank\" href=\"{{ asset('') }}"+item.url_foto+"\"><img src=\"{{ asset('') }}"+item.url_foto+"\"></a>";
        // console.log("{{ asset('') }}"+item.url_foto)
    });

    $(".carousel").html(htmlData);
    $('.carousel').carousel();
}

function deleteData(id){
    console.log(id);
    dataHtml = "<form method=\"POST\" action=\"{{ url('backend/hotel_fasilitas') }}/"+id+"/delete\" accept-charset=\"UTF-8\">";
...
<div id="zoomFoto" class="modal" style="max-height: 80%;">
    <div class="modal-content">
        <!-- <img src="{{ asset('uploads/default.jpg') }}" id="srcZoomFoto" width="100%" /> -->
        <div class="carousel">
            <a class="carousel-item" href="#one!"><img src="{{ asset('uploads/default.jpg') }}"></a>
        </div>
    </div>
