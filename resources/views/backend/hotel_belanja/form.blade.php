...
<div class="col m12">
    ...
    <div class="row">
        <div class="col s12{{ $errors->has('jumlah_kamar') ? ' has-warning' : '' }}">
            ...
            <select id="hotel" name="hotel" class="browser-default">
            @foreach(\App\Hotel::get() as $hotel)
                <option value="{{ $hotel->id }}"{{ (@$data->hotel_id == $hotel->id)? ' selected' : '' }}>{{ ucwords($hotel->nama_hotel) }}</option>
            @endforeach
            </select>
    ...
    <div class="row" id="cbPaketWisata">
        <div class="col s12">
            ...
            <select id="wisata_paket" name="wisata_paket" onchange="hitungJumlahPaket(this)" class="browser-default">
                ...
                @php
                $belanjaWisataPaket = \App\HotelBelanjaPaketWisata::where('hotel_belanja_id', $data->id)->first();
                @endphp
                
                @forelse(\App\HotelPaketWisata::where('hotel_id', $data->hotel_id)->get() as $wp)
                <option value='{{ $wp->id }}'{{ ($belanjaWisataPaket->hotel_paket_wisata_id==$wp->id) ? ' selected' : '' }}>{{ $wp->nama }} (Rp. {{ $wp->harga }})</option>
                @empty
    ...
    <div class="input-field col s12">
        ...
        <a href="{{ route('backend::hotel_belanja.index', ['tipe' => $tipe_wisata]) }}" class="btn waves-effect waves-light left" role="button">Batal
    ...
    <!-- <div class="form-group">
        ...
        <a href="{{ route('backend::hotel_belanja.index', ['tipe' => $tipe_wisata]) }}" class="btn btn-primary btn-sm" role="button">Batal</a>
    </div> -->
</div>
...
<script>
    ...
    var hotelId = {{ \App\Hotel::first() ? \App\Hotel::first()->id : 0 }};
    ...
    $("select#kategori").change(function(){
        ...
        if(kat=='paketwisata'){
            ...
            listPaketWisata('{{ \Auth::user()->hotel->id }}');
            @else
            listPaketWisata(hotelId);
            @endif
    ...
    function listPaketWisata(idHotel){
        $.ajax({
            type:'GET',
            url:"{{ url('getListHotelPaketWisata') }}/"+idHotel,
            // data:"id=" + propinsi,
