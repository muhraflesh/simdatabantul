...
@section('breadcrumbs')
    {!! Breadcrumbs::render('list_belanja_'.$tipe_wisata) !!}
@endsection

@section('action')
<div class="col s2 m6 l6">
    <a class="btn waves-effect waves-light breadcrumbs-btn right" href="{{ route('backend::hotel_belanja.create', ['tipe'=>$tipe_wisata]) }}">
...
<!-- <div class="col s2 m6 l6">
    ...
    <ul class="dropdown-content" id="dropdown1" tabindex="0">
        <li tabindex="0">
            <a class="grey-text text-darken-2" href="{{ route('backend::hotel_belanja.create', ['tipe'=>$tipe_wisata]) }}">
                Tambah Data
            </a>
        </li>
    ...
    <div class="col s12 m12 l12">
        <div id="responsive-table" class="card card card-default scrollspy">
            <div class="card-content">
                ...
                <form class="col s12" action="{{ route('backend::hotel_belanja.index', ['tipe'=>$tipe_wisata]) }}" method="GET">
                    <div class="row">
                        ...
                        <div class="input-field col m3 s6">
                            <select name="kategori" class="browser-default">
                                <option{{ \Request::get('kategori')==''? " selected" : "" }} value="">Semua</option>
                                <option value="kuliner"{{ (\Request::get('kategori') == 'kuliner')? ' selected' : '' }}>Kuliner</option>
                                <option value="oleholeh"{{ (\Request::get('kategori') == 'oleholeh')? ' selected' : '' }}>Oleh-Oleh</option>
                                <option value="transportasi"{{ (\Request::get('kategori') == 'transportasi')? ' selected' : '' }}>Transportasi</option>
                                <option value="paketwisata"{{ (\Request::get('kategori') == 'paketwisata')? ' selected' : '' }}>Paket Wisata</option>
                            </select>
                        </div>
            ...
            <div class="row">
                ...
                <table class="responsive-table">
                    ...
                    <tr>
                        ...
                        <th data-field="jumlah">Jumlah Belanja</th>
                        <th data-field="total">Total Belanja</th>
                        <th data-field="kategori">Kategori Belanja</th>
                        @if(\Auth::user()->hasRole('superadmin'))
                    ...
                    <?php $no=1;?>
                        ...
                        <tr>
                            ...
                            <td>
                            @if($res->kategori_belanja=='oleholeh')
                                Oleh-Oleh
                            @elseif($res->kategori_belanja=='paketwisata')
                                Paket Wisata
                            @else
                                {{ ucwords($res->kategori_belanja) }}
                            @endif
                            </td>
                            @if(\Auth::user()->hasRole('superadmin'))
                            <td>
                                {{ $res->hotel->nama_hotel }}
                            </td>
                            @endif
                            <td>
                                ...
                                <ul id='dropdown{{ $no }}' class='dropdown-content'>
                                    ...
                                    <li>
                                        <a href="{{ route('backend::hotel_belanja.edit', ['tipe'=>$tipe_wisata, 'id' => $res->id]) }}">
...
function deleteData(id){
    console.log(id);
    dataHtml = "<form method=\"POST\" action=\"{{ url('backend/hotel_belanja/'.$tipe_wisata) }}/"+id+"/delete\" accept-charset=\"UTF-8\">";
